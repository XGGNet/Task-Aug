...
def train(model, saver, sess, exp_string, train_file_list, test_file, resume_itr=0):
    ...
    domain_list = ['VGH', 'NKI', 'IHC', 'NCH']
    for d,domain in enumerate(range(len(domain_list))):
        print(domain_list[d])
        test_file = [os.path.join('./JMI', domain_list[d]+'_list.txt')]
        test_data = ImageDataGenerator(test_file, dataroot=FLAGS.dataroot, mode='inference', \
                                        batch_size=1, num_classes=FLAGS.num_classes, shuffle=False)
        test_iterator = tf.data.Iterator.from_structure(test_data.data.output_types, test_data.data.output_shapes)
        test_next_batch = test_iterator.get_next()
    ...
    # if True:#itr % FLAGS.test_print_interval == 0:
        test_acc, test_loss, test_count = 0.0, 0.0, 0.0
        semantic_features = []
        # domain_labels = []
        class_labels = []
        ...
        for it in range(test_batches_per_epoch):
            test_input, test_label = sess.run(test_next_batch)
            feed_dict = {model.test_input: test_input, model.test_label: test_label, model.KEEP_PROB: 1.}
            output_tensors = [model.test_loss, model.test_acc, model.semantic_feature, model.y_pred, model.y_conf]
            result = sess.run(output_tensors, feed_dict)
            test_loss += result[0]
            test_acc += result[1]

            semantic_features.append(result[2][0])
            class_preds.append(result[3][0])
            class_confs.append(result[4][0])

            test_count += 1
            this_class = np.argmax(test_label, axis=1)[0]
            
            class_labels.append(this_class)

            class_accs[this_class] += result[1] # added for debug
            class_samples[this_class] += 1
        test_acc = test_acc/test_count
        print(f'current_domain:{test_file}, len:{test_batches_per_epoch}, test_acc:{test_acc:.5f}')

        path = './vis_results/tsne2/'
        if not os.path.exists(path):
            os.makedirs(path)
        feature_file = path + 'deepall_VGH_X.csv'
        label_file = path + 'deepall_VGH_labels.csv'

        # semantic_features = np.concatenate(semantic_features)
        # class_preds = np.concatenate(class_preds)
        # class_labels = np.concatenate(class_labels)
        assert len(semantic_features) == len(class_preds) == len(class_labels) == len(class_confs)
        domain_labels = [domain_list[d] for _ in range(len(semantic_features))]
        
        data = {'domain_labels':domain_labels,'class_labels':class_labels,'class_preds':class_preds,'class_confs':class_confs}
        df_data = pd.DataFrame(data)
        df_data.to_csv(label_file,mode='a')

        df_data = pd.DataFrame(semantic_features)
        df_data.to_csv(feature_file,mode='a')

        # with h5py.File('./vis_results/tsne/deepall_VGH2.h5',"w") as file:
        #     file.create_dataset(domain_list[d],data=semantic_features)
        #     data['semantic_features'] = semantic_features
        #     data['domain_labels'] = domain_labels
        #     data['class_labels'] = class_labels
        #     data['class_preds'] = class_preds
        #     file.create_dataset(domain_list[d],data=data)
...
def main():
    ...
    resume_itr = 0
    checkpoint_dir = ''
    checkpoint_dir = './log/mame_VGH/deepall_mbs_120.inner5e-05.outer5e-05.clipNorm2.0.metric1e-05.margin20.0/'
    checkpoint_file = 'checkpoint_run2'
    if FLAGS.resume or not FLAGS.train:
        model_file = tf.train.latest_checkpoint(checkpoint_dir,checkpoint_file)
        if model_file:
    ...
    test_file_list = [os.path.join(filelist_root, source_domain+'_list.txt') for source_domain in source_list]
