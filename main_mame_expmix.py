...
flags.DEFINE_string('mixway', '', 'meta-test por: 0(a) [0,0.5](b) [0.5,1](c) [0,1](CET) 1(ET)')
...
def train(model, saver, sess, exp_string, train_file_list, test_file, resume_itr=0):
    ...
    for itr in range(resume_itr, FLAGS.train_iterations):
        ...
        if FLAGS.method=='deepall':
            ...
            if FLAGS.method=='MET':
            # exp for mix, meta_test por: 0 [0,0.5] [0.5,1] [0,1](CET) 1(ET)
                if FLAGS.mixway=='a':
                    # por~0
                    sum = 10
                    ...
                    input_target_1 = np.concatenate((inputa[:part_a], inputa1[:part_b]), axis=0)
                    label_target_1 = np.concatenate((labela[:part_a], labela1[:part_b]), axis=0)
                elif FLAGS.mixway=='b':
                    #por~[0,0.5]
                    sum = 10
                    part_c = random.sample([0,3,5],1)[0]
                    ...
                    #por~[0.5,1]
                    sum = 10
                    part_c = random.sample([5,7,10],1)[0]
                    part_b = random.randint(0, sum - part_c)
                    part_a = sum - part_c - part_b
