...
def train(model, saver, sess, exp_string, train_file_list, test_file, resume_itr,vissave_path):
    ...
    for it in range(int(test_batches_per_epoch)):
        ...
        test_case = test_filename[0].decode().split('/')[-2]
        test_slice = int(test_filename[0].decode().split('/')[-1].split('.')[0].replace('image',''))
        test_gt = cv2.imread(test_filename[0].decode().replace('image','label'),0)
        y_gt.append(test_gt[np.newaxis,...])
        if current_case==None:
            current_case = test_case
            cs_cnt = 1
            case_index.append(0)
    ...
    print(f'current_domain:{test_file}, len:{test_batches_per_epoch}, test_dice:{test_masked_dice:.5f}')

    for k in range(len(y_true)):
        img = np.uint8(x_input[k]*255)
        pred = np.uint8(y_pred_post[k]*255)
        gt = cv2.resize(y_gt[k],(128,128))
        gt = np.where(gt>127,255,0)
        save_img = np.hstack((img,pred,gt))

        slice_dice = np_dice(y_true=y_true[k], y_pred=y_pred_post[k])
        cv2.imwrite(f'{vissave_path}/{k}_{slice_dice:.4f}.png',save_img)
...
def main():
    ...
    FLAGS.target_domain = 'IRCAD'
    perform = '0.8256'

    vissave_path = f'./vis_results/liver_seg/{FLAGS.target_domain}/'
    vissave_path += f'{perform}/'
    if not os.path.exists(vissave_path):
        os.makedirs(vissave_path)

    resume_itr = 0
    checkpoint_dir = f'./vis_results/liver_seg/{FLAGS.target_domain}/model_1/'
    checkpoint_file = 'checkpoint_warmup_mask_run3'
    ...
    train(model, saver, sess, exp_string, train_file_list, test_file_list, resume_itr,vissave_path)
